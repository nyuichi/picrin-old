...
static bool already_initialized_symbols_p = false;

static pic_obj_t PIC_SYMBOL_QUOTE;
static pic_obj_t PIC_SYMBOL_QUASIQUOTE;
static pic_obj_t PIC_SYMBOL_UNQUOTE;
static pic_obj_t PIC_SYMBOL_UNQUOTE_SPLICING;
...
typedef struct {
  pic_obj_t port;
  pic_tokenizer_t token;
  bool backtrack;
} pic_parser_t;


#define USE_PARSER pic_parser_t * parser
...
static void get_symbol(USE_PARSER)
{
  ...
  KIND = T_SYMBOL;
  PIC_XDECREF(DATA);
  DATA = pic_get_interned_symbol(str);
}
...
{
  if (!already_initialized_symbols_p) {

    PIC_SYMBOL_QUOTE = pic_get_interned_symbol("quote");
    PIC_SYMBOL_QUASIQUOTE = pic_get_interned_symbol("quasiquote");
    PIC_SYMBOL_UNQUOTE = pic_get_interned_symbol("unquote");
    PIC_SYMBOL_UNQUOTE_SPLICING = pic_get_interned_symbol("unquote-splicing");

    already_initialized_symbols_p = true;
  }
...
static void pic_write_pair(pic_obj_t obj, pic_obj_t port)
{
...
void pic_write(pic_obj_t obj, pic_obj_t port)
{
  ...
  if (PIC_FIXNUMP(obj)) {
    ...
    if (PIC_SYNTAXP(obj)) {
      fprintf(file, "#<syntax %d>", (int)obj>>8);
      return;
    }
  }
